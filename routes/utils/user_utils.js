...
async function markAsFavorite(user_id, recipe_id){
    await DButils.execQuery(`insert into mydb.FavoriteRecipes values ('${user_id}','${recipe_id}')`);
}

async function getFavoriteRecipes(user_id){
    const recipes_id = await DButils.execQuery(`select recipe_id from mydb.FavoriteRecipes where username='${user_id}'`);
    return recipes_id;
}
...
async function createRecipes(imageUrl, title, readyInMinutes, popularity, vegan, vegetarian, gluten_free, ingredients, instructions, servings, user_name){
    ...
    for (let z = 0; z < ingredients.length; z++){
        console.log("ingredients[i] "+ingredients[z]);
        console.log("id2: " + id);
        await DButils.execQuery(`insert into mydb.ingredientsrecipes values (${id}, '${ingredients[z]}')`);
    }
...
async function isFavoriteRecipe(user_name, recipe_id){
    const count_favorites = await DButils.execQuery(`SELECT count(*) AS amount FROM mydb.FavoriteRecipes WHERE username='${user_name}' AND recipe_id='${recipe_id}'`);
...
async function getMyFamilyRecipes(user_name){
    try {
        const recipesIds = await DButils.execQuery(`SELECT * from mydb.FamilyRecipes where username='${user_name}'`);
        return recipesIds;
    }
    catch(err){
